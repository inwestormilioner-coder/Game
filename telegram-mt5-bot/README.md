...
- **Siatka wszystko-albo-nic** (`ABANDONED_GRID_CANCEL=true`, domyślnie
  włączone): jeśli jedno zlecenie oczekujące z danej strefy zniknie BEZ
  fillu (np. ręcznie usunięte w terminalu, wygasłe, odrzucone), bot
  automatycznie usuwa resztę jeszcze niewypełnionych zleceń z tej samej
  strefy (`Mt5Executor.detect_abandoned_grid`) - niekompletna siatka
  przestaje reprezentować zakładaną wielkość/ryzyko strefy. Zlecenie,
  które zniknęło bo się wypełniło (normalny przypadek), zostaje bez
  zmian razem z resztą siatki. Działa w obu `EXIT_MODE`. Ustaw
  `ABANDONED_GRID_CANCEL=false`, żeby wyłączyć to per instancja bota (np.
