...
- **Melee/Ranged physical damage** = `WeaponBaseDamage × (1 + WeaponSkillLevel / 100) × EquipmentModifiers`
...
**Visual target:** modern stylized-realism 3D fantasy — detailed but readable character silhouettes (critical for fast target identification on small phone screens), atmospheric lighting per biome, and impressive-but-performant spell VFX, all original art direction (no visual resemblance to Tibia's 2D/2.5D retro aesthetic or to any existing MOBA's character/map designs).
