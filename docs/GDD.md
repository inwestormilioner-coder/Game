...
- **Currency:** Gold Coins → Sapphire Coins → Arcane Coins, 100:1 at each step (100 Gold = 1 Sapphire, 100 Sapphire = 1 Arcane). Deliberately **not** a HUD number — coins are physical, stackable inventory items with real weight, exactly like any other loot: carried in the backpack, stored in the Depot, dropped, or traded, and subject to the same carry-capacity limit as everything else. Spending (Player.trySpendGold) breaks down and re-consolidates coins the way real change-making works — pay a 10,000-gold Arcane Coin for a 1,000-gold cost and get 90 Sapphire Coins back, not 9,000 loose Gold Coins.
...
Each profession needs a **tool equipped in the weapon slot** (a pickaxe for Mining, a hatchet for Woodcutting, a rod for Fishing — Fishing additionally consumes one unit of **bait** carried in the backpack per cast, tiered the same way). There's deliberately no separate "interact with this node" prompt: equipping the tool swaps out whatever combat weapon was there, and gathering then happens through the **same single tap-to-target-and-attack flow as everything else in the game** — walk up to an ore vein/tree/fishing spot, it becomes the nearest target exactly like a monster, a lootable corpse, or an NPC would (Section 17), the one action button relabels to the profession's verb (Kop/Rąb/Łów), and tapping it gathers instead of swinging a sword. Gathering nodes (ore veins, trees, fishing spots) come in **tiers**, and two independent things gate a successful gather:
...
**Current build status:** two tiers per profession are in and playable — a tier-1 node anyone can start on (Copper vein, Birch tree, a shallow fishing spot) and a tier-2 node needing both level 15 and the better tool (Iron vein, Oak tree, a deep spot). Every class starts with a basic tier-1 toolkit (rusty pickaxe/hatchet, a simple rod, 5 earthworms) in the backpack, unequipped, so gathering is reachable from minute one by equipping whichever tool you want to use — there's no general-goods shop yet (Section 32) to buy tiered tools/bait from, so tier-2 gear is debug/loot-only for now. Fish also double as food (Section 6) — a nice free synergy, since they were always going to be edible. What to actually *do* with ore/logs (smithing, carpentry — a full crafting system) is intentionally still undecided, per the original ask: gather first, decide the crafting sinks later.
...
| Large Gold Coin Pouch | Guaranteed | 100% |
...
- **Travel:** overland is on-foot (+ mounts later); inter-island/continent travel uses **ships as physical world objects** — the player walks to a dock, talks to a Ferryman NPC, pays a Gold Coin fare, and a travel sequence (a short, real transit — not an instant cut) carries them to the destination dock. Later game stages introduce faster options (teleportation circles, wizard-cast group teleport spells, rare artifact-based fast travel) but these are always *unlocked, costly, or limited*, never replacing the base experience of a new player's first sea crossing.
...
**Current build status:** a quest-giver (Elder Mara), a Depot keeper (Orin), and a two-way Ferryman pair are in and playable. There is deliberately **one action button in the whole game**, not one per interaction type: NPCs compete for it in the same nearest-target race as monsters, corpses, and gathering nodes (Section 5's professions), relabeling to "Rozmawiaj"/"Depozyt" exactly the way it relabels to "ATAK"/"SZUKAJ"/"Kop" — walking up to Elder Mara and tapping the button talks to her the same way walking up to a wolf and tapping it attacks. Simplicity over adding more buttons was an explicit design call here. The general-goods shop (buy/sell) from the MVP scope (Section 32) isn't built yet — dialogue trees today only branch on quest state (offer / in-progress / ready-to-turn-in / already done), not on prior free-form answers.
...
- **Bounty system:** any player may place a Gold Coin bounty on a Wanted player's head, claimable by whoever kills them next (a player-driven "wanted poster" economy loop).
...
**Current build status:** the Knight's model is in and playable — a Meshy.ai export (10.4k triangles, well inside a mobile budget) rigged with a standard biped skeleton, confirming the chibi/stylized-fantasy direction reads correctly in-engine. Eight animation clips (Idle, two walk variants, Run, two attack variants, Hit Reaction, Death) were exported separately, each a full duplicate of the mesh+skeleton+texture (a Meshy export quirk), then programmatically merged into one file sharing a single mesh/skeleton/texture — collapsing ~55MB of duplicated exports down to ~1.4MB, most of that saved by re-encoding the 2048×2048 source texture down to 1024×1024 WebP. Player.ts drives a small priority-ordered animation state machine (Death > Attack/Hit one-shots > Walk/Idle loop) via a standard `THREE.AnimationMixer`; classes without a model yet keep the original placeholder primitive mesh as an automatic fallback. The remaining four classes' models/animations are the next content item, following the same export → merge → wire pipeline.

Monsters now use the same asset pipeline and the same `Monster.ts` animation state machine as the Knight (`mesh` is a `THREE.Group` swapped from a placeholder sphere to a loaded model, same Death > Attack/Hit-one-shot > Walk/Idle priority order). The Ashfen Goblin is the first one in: Meshy.ai's own Animate feature doesn't cover every creature type, so when it came back empty for the goblin, the fallback pipeline was a static Meshy export → a hand-extracted OBJ (no Blender in the build environment; geometry/UVs/texture pulled directly from the glTF accessors) → Mixamo for auto-rigging and animation (Idle, Walk, Attack, Hit Reaction, Death) despite the source mesh being frozen in a non-neutral combat pose rather than a T-pose → the same merge/compress pipeline, adjusted for Mixamo's `mixamorig:`-prefixed joint names. One gotcha specific to this path: Mixamo read the source OBJ's meter-scale units as centimeters, coming back 100x too small — corrected with a documented `modelScale` field on `MonsterDef` (applied at load time) rather than baked into the asset. The remaining monster types, and the four non-Knight classes, are next in line for the same treatment.

**Camera:** third-person, player-centered, smooth-follow, angled downward (~50°) for a wide isometric-style view of the surrounding area (reference: a mobile isometric ARPG town square) rather than a close chase camera — tuned down once from an initial, too-steep-per-real-device-testing pass, and final character art will inform further tuning once it's in. **Both the angle and the distance are fixed and identical for every player — there is no player-controlled rotation and no zoom on the 3D camera.** This is a fairness decision, not an oversight: a rotatable or zoomable camera would let a player peek around obstacles, or simply see further across the battlefield, in a way other players standing at the same spot cannot — and an open-world PvP game is full of moments where two players are looking at the very same fight and that gap would matter. A fixed camera also keeps "north on the minimap" and "up on screen" meaning the same thing for everyone, which matters for the Find-Player spell's directional callouts (Section 22) and for describing locations to guildmates in chat. Seeing further via the *minimap's* zoom is still the intended lever for that (Section 10, at a fixed equal cost to every player) — the wider combat-camera framing is about legibility of the immediate area, not a substitute for it. Framing is tuned for wide peripheral visibility around the player at its one fixed distance (reading incoming melee/ranged threats and AoE telegraphs in combat) — a mobile-MOBA-style camera *behavior*, built entirely with original assets and framing choices.

**Aimed-attack joystick:** the single action button (Section 17) got a second, optional layer on top of it rather than a replacement — a quick tap still attacks whichever target the existing nearest-of-any-type race picked, unchanged. Pressing and dragging it instead (mobile-MOBA reference: Mobile Legends' basic-attack stick) shows a translucent range-and-cone reticle on the ground and lets the player pick a monster by direction — useful once several monsters are in range at once, when "nearest" isn't necessarily "the one I'm actually trying to hit." Releasing attacks whichever alive monster falls inside the cone, or nothing at all if the cone is empty (a "Brak celu w tym kierunku" toast, not a silent miss). This only ever applies to monster combat — corpses, gathering nodes, and NPCs stay pure walk-up-and-tap, since aiming a direction at a stationary thing doesn't mean anything.
